...
import { WORLD_MAP_BLOCK_SIZE, WORLD_MAP_BLOCK_SIZE_POWER } from "../../../vars";

export class MapCollisions {

  /**
   * @param {THREE.Vector3|Vector3} position
   * @param {THREE.Vector3|Vector3} shiftVector
   * @param {*} options
    ...
    if (shiftX) {
      ...
      for (let blockX = currentBorderBlockPosition.x - sign * 2; blockX !== nextBorderBlockPosition.x; blockX += sign) {
        ...
        blocksValues = blocksPositions.map(level => {
          return level.map( blockPosition => map.hasBlock( ...blockPosition.toArray() ) );
        });
  ...
  /**
   * @param {THREE.Vector3|Vector3} currentPosition
   * @param {number} shiftY
   * @param {*} options
